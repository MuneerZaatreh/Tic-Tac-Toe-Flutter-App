...
GetEmptyPostion(List gamePad) {
  int placei = Random().nextInt(3);
  int placej = Random().nextInt(3);
  while (gamePad[placei][placej] != '.') {
    placei = Random().nextInt(3);
    placej = Random().nextInt(3);
  }
  UpdateStatus(placei, placej);
}
...
class _SoloState extends State<Solo> {
  bool GameOver() {
    if (whoWins().toString() == 'X' || whoWins().toString() == 'O') {
      return false;
    }
    return true;
  }
